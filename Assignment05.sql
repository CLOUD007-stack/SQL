...
INSERT INTO franchises(franchise_name) VALUES ('DC Franchise');
INSERT INTO franchises(franchise_name) VALUES ('MCU Franchise');
...
UPDATE features SET franchise_id = 4 WHERE title IN (
    "Man of Steel", "Superman Returns",'Superman Vs. Batman'
);
...
UPDATE features SET franchise_id = 6 WHERE title IN (
...
#Ans 1
SELECT feature_id, title, year
FROM features
WHERE franchise_id = (
    SELECT franchise_id
    FROM franchises
    WHERE franchise_name = 'James Bond Franchise'
);
...
SELECT franchise_name
FROM franchises
JOIN features ON franchises.franchise_id = features.franchise_id
GROUP BY franchises.franchise_id, franchises.franchise_name
HAVING COUNT(features.feature_id) = 1;

#Ans 2
SELECT DISTINCT feature_role.person AS Person,
                feature_role.role AS MarvelChar,
                features.title AS MarvelMovie
FROM feature_role
JOIN features  ON feature_role.feature_id = features.feature_id
JOIN franchises ON features.franchise_id = franchises.franchise_id
WHERE franchises.franchise_name = 'MCU Franchise'
AND feature_role.person IN (
    SELECT person
    FROM feature_work
    WHERE job = 'actor' AND feature_id IN (
        SELECT feature_id
        FROM features
        WHERE franchise_id = (
            SELECT franchise_id
            FROM franchises
            WHERE franchise_name = 'Sherlock Holmes Franchise'
        )
    )
)
ORDER BY feature_role.person;

#Ans 3
SELECT franchises.franchise_name, features.title AS titleMovie
FROM franchises 
JOIN features  ON franchises.franchise_id = features.franchise_id
JOIN feature_role ON features.feature_id = feature_role.feature_id
WHERE feature_role.person = 'Henry Cavill'
GROUP BY franchises.franchise_id, franchises.franchise_name, features.title
HAVING COUNT(DISTINCT features.feature_id) = 1;

#Ans 4
SELECT DISTINCT
    f1.franchise_name AS franchise_f1,
    f2.franchise_name AS franchise_f2,
    fe1.title AS movie_m1,
    fe2.title AS movie_m2,
    fw1.person AS cmn_actor
FROM 
    franchises f1
JOIN 
    features fe1 ON f1.franchise_id = fe1.franchise_id
JOIN 
    feature_work fw1 ON fe1.feature_id = fw1.feature_id
JOIN 
    features fe2 ON fe1.feature_id <> fe2.feature_id 
JOIN 
    franchises f2 ON fe2.franchise_id = f2.franchise_id
JOIN 
    feature_work fw2 ON fe2.feature_id = fw2.feature_id
WHERE 
    fw1.person = fw2.person 
    AND f1.franchise_id < f2.franchise_id 
    AND fe1.title <> fe2.title; 
    
-- Assignment 05--

-- Ans 1
CREATE VIEW character_types AS
SELECT
    fr.role AS role,
    rt.role_type,
    COUNT(fr.feature_id) AS times
FROM
    feature_role fr
JOIN
    role_type rt ON fr.role_id = rt.role_id
GROUP BY
    fr.role, rt.role_type;

select * from character_types;

-- Ans 2 
CREATE VIEW type_affinities AS
SELECT fe.person, r.role_type, COUNT(*) AS Affinity
FROM feature_role fe
JOIN role_type r ON fe.role_id = r.role_id
GROUP BY fe.person, r.role_type
HAVING Affinity > 0
ORDER BY fe.person, r.role_type;

select * from type_affinities;

-- Ans 3
CREATE VIEW type_casting AS
select fe.person, r.role_type from type_affinities ta
join feature_role fe on fe.person = ta.person
join role_type r on r.role_id = fe.role_id and ta.role_type = r.role_type 
group by fe.person, r.role_type having 
    COUNT(DISTINCT r.role_type) = 1
    AND COUNT(DISTINCT fe.feature_id) > 1;
    
select * from type_casting;

-- Ans 4
CREATE VIEW casting_ideas AS
SELECT 
    potential_roles.role AS role,
    actor_affinities.person AS person,
    SUM(actor_affinities.affinity_score) AS credibility_score
FROM 
    (SELECT 
        fr.role, 
        rt.role_type
     ...
     JOIN 
        role_type rt ON fr.role_id = rt.role_id
     GROUP BY 
        fr.role, rt.role_type) AS potential_roles
JOIN 
    (SELECT 
        fw.person, 
        rt.role_type, 
        COUNT(DISTINCT fw.feature_id) AS affinity_score
     FROM 
        feature_work fw
     JOIN 
        feature_role fr ON fw.feature_id = fr.feature_id AND fw.person = fr.person
     JOIN 
        role_type rt ON fr.role_id = rt.role_id
     GROUP BY 
        fw.person, rt.role_type) AS actor_affinities ON potential_roles.role_type = actor_affinities.role_type
GROUP BY 
    potential_roles.role, actor_affinities.person
HAVING 
    SUM(actor_affinities.affinity_score) > 1;

-- Ans 5 
select person from casting_ideas where role = 'James Bond' order by credibility_score desc LIMIT 1;
select person from casting_ideas where role = 'Tony Stark' order by credibility_score desc LIMIT 1;
select person from casting_ideas where role = 'Sherlock Holmes' order by credibility_score desc LIMIT 1;
SELECT person FROM casting_ideas WHERE role = 'Robin Hood' ORDER BY credibility_score DESC LIMIT 1;
